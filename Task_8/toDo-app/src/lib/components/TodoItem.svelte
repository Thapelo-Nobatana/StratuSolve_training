...
<li class="bg-gray-300 p-3 rounded">
  {#if isEditing === true}
     ...
     <div class="flex gap-2">
        <button class="flex-1 bg-blue-600 text-white p-2 rounded" onclick={save}>Save</button>
        <button class="flex-1 bg-gray-500 text-white  p-2 rounded" onclick={() => isEditing = false}>Cancel</button>
     </div>
     {:else}
